...
export default async function handler(req, res){
    ...
    if(req.method === "GET"){
        ...
        const contFacturacion = await query(
            {
                query: "SELECT * FROM datos_facturacion WHERE fecha_cont_facturacion=? AND hora_cont_facturacion=? ",
    ...
    */
    ...
    if (req.method === "DELETE") {
        const FechaContFacturacion = req.body.fecha_cont_facturacion;
        const HoraContFacturacion = req.body.hora_cont_facturacion;
        const deleteContFacturacionTodo = await query({
          query: "DELETE FROM datos_facturacion WHERE fecha_calFacturacion=? AND hora_calFacturacion=?",
          values: ([FechaContFacturacion, HoraContFacturacion]),
        });
        const result = deleteContFacturacionTodo.affectedRows;
        if (result) {
          message = "success";
        } else {
          message = "error";
        }
        res.status(200).json({ response: { message: message, facturacion: deleteContFacturacionTodo } });
      }
}