...
export default async function handler(req, res){
    ...
    if(req.method === "GET"){
        ...
        const almacen = await query(
            {
                query: "SELECT * FROM material_almacen WHERE id_materialAlmacen=? OR cantidad_materialAlmacen=? OR nombre_materialAlmacen=? OR tipo_materialAlmacen=? OR observaciones_materialAlmacen=? ",
