...
export default async function handler(req, res){
    ...
    const { comprador, fecha, hora } = req.query;

    if(req.method === "GET"){

        const facturacion = await query(
            {
                query: "SELECT * FROM datos_facturacion WHERE comprador_calFacturacion = ? AND fecha_calFacturacion = ? AND hora_calFacturacion = ?",
                values: ([comprador, fecha, hora])
            }
        );
